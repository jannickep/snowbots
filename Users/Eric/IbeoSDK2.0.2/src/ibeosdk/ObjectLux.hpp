...
class ObjectLux : public Serializable {
public:
	ObjectLux();
	ObjectLux(const ObjectLux& src);
	virtual ~ObjectLux();

	ObjectLux& operator= (const ObjectLux& src);
public:
...
public:
	...
	const Point2d& getReferencePoint() const { return m_refPoint; }
	Point2d getReferencePoint() { return m_refPoint; }
	const Point2d& getReferencePointSigma() const { return m_refPointSigma; }
	Point2d getReferencePointSigma() { return m_refPointSigma; }

	const Point2d& getClosestPoint() const { return m_closestPoint; }
	Point2d getClosestPoint() { return m_closestPoint; }


	Point2d getBoundingBoxCenter() { return m_boundingBoxCenter; }
	...
	Point2d getObjectBoxCenter() { return m_objectBoxCenter; }
	const Point2d& getObjectBoxCenter() const { return m_objectBoxCenter; }
	...
	Point2d getAbsoluteVelocity() { return m_absVelocity; }
	const Point2d& getAbsoluteVelocity() const { return m_absVelocity; }
	UINT16 getAbsoluteVelocitySigmaX() const { return m_absVelSigmaX; }
	UINT16 getAbsoluteVelocitySigmaY() const { return m_absVelSigmaY; }

	Point2d getRelativeVelocity() { return m_relVelocity; }
	const Point2d& getRelativeVelocity() const { return m_relVelocity; }
...
public:
	...
	void setRefPointSigma(const Point2d refPointSigma) { this->m_refPointSigma = refPointSigma; }
	...
	void setObjectBoxWidth(const UINT16 objectBoxWidth) { this->m_objectBoxSizeX = objectBoxWidth; }
	void setObjectBoxLength(const UINT16 objectBoxLength) { this->m_objectBoxSizeY = objectBoxLength; }
...
public:
	static float angle2rad(const INT16 ticks)
	{
		...
		// (x < 0) ? ((x % N) + N) : x % N
		return float(((ticks < 0) ? ((ticks % angleTicksPerRotation) + angleTicksPerRotation)
		                          : ticks % angleTicksPerRotation)
